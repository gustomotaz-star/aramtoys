...
export default function OrderConfirmationPage(){
  ...
  useEffect(()=>{(async()=>{const {data,error:queryError}=await supabase.from('orders').select('id,total,shipping_fee,payment_method,status,created_at,order_items(product_name,quantity,line_total),addresses(full_address,city,governorate)').eq('id',orderId).single(); if(queryError)setError(queryError.message);else setOrder(data);})();},[orderId]);
  return <main className="confirm-wrap">
    ...
    {order&&<div className="confirm-card">
      {order.order_items?.map((item,index)=><div className="legacy-row" key={`${item.product_name}-${index}`}><span>{item.product_name} × {item.quantity}</span><span>{formatMoney(item.line_total)}</span></div>)}
